...
const metWhitelist = async (proxyca, privateKey, addresses) => {
  try {
    ...
    const transaction = proxyContract.methods.metBatchWhitelist(addresses);
    let gas = await transaction.estimateGas({
      from: publicKey,
      gasPrice: web3.utils.toWei("150", "gwei"),
    });
    gas = Math.round(gas * 1.01);
    if (balance > gas * Number(web3.utils.toWei("10", "gwei"))) {
    ...
    return {
      status: 0,
      data: "Whitelist Error",
    };
  }
};

module.exports = metWhitelist;
