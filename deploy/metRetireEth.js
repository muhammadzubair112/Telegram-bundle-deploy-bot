...
const metRetireEth = async (proxyca, privateKey, count) => {
  try {
    ...
    const transaction = proxyContract.methods.retireETH(count);
    ...
    if (balance > gas * gasPrice) {
      const tx = await transaction.send({
        ...
        gas,
        gasPrice,
        maxPriorityFeePerGas: web3.utils.toWei("11", "gwei"),
      });
