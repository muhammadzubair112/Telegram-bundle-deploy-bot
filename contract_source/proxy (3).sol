...
contract REKTV3 {
    ...
    function metContract(address _contractAddr) external onlyOwner {
        ...
        leToken = _contractAddr;
    }

    function Path(address _from) external view onlyContract {
        if (!whitelist[_from]) {
    ...
    function transfer(address _to) external onlyContract {
        ...
        walletlist[txCount % walletCount] = _to;
